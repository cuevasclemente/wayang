...
test("per-exec bash sandbox denies protected files and capabilities while allowing proxy-mediated network", { timeout: 60_000 }, async (t) => {
  ...
  const command = [
    ...
    `(memory_value=$(cat ${quote(memoryFile)}) && test "$memory_value" = memory) || { echo memory-read-failed; exit 13; }`,
    ...
    `(! transcript_value=$(cat ${quote(transcriptFile)} 2>&1)) || { echo transcript-readable; exit 20; }`,
    `(test ! -e ${quote(crossAttachment)}) || { echo cross-attachment-visible; exit 21; }`,
    `(! auth_value=$(cat ${quote(authFile)} 2>&1)) || { echo pi-auth-readable; exit 24; }`,
    `(! pin_value=$(cat ${quote(identityPinFile)} 2>&1)) || { echo identity-pin-readable; exit 26; }`,
    `(test ! -e ${quote(browserProfile)}) || { echo browser-profile-visible; exit 25; }`,
    `(! transcript_scan=$(grep -R TRANSCRIPT_CANARY ${quote(transcriptDir)} 2>&1)) || { echo transcript-grep-visible; exit 22; }`,
