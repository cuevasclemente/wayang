...
function QueuedUserMessages({
  ...
  const needsReview = messages.some((message) => message.cancelStatus === "uncertain");

  return (
    <div className="border-t border-neutral-800 bg-neutral-950/95 px-3 py-2">
      <button
        type="button"
        data-testid="chat-queued-toggle"
        onClick={() => setCollapsed((value) => !value)}
        aria-expanded={!collapsed}
        className="mb-1 flex w-full items-center gap-2 text-left text-[10px] font-semibold uppercase tracking-wider text-amber-300"
      >
        <span className="h-2 w-2 rounded-full bg-amber-400 animate-pulse" />
        {needsReview ? "Pending messages — delivery needs review" : `Queued for the next turn (${messages.length})`}
        <span className="ml-auto shrink-0 text-amber-500/80">{collapsed ? "Show" : "Hide"}</span>
      </button>
      {!collapsed && (
      <div className="max-h-[28dvh] space-y-1 overflow-y-auto overscroll-contain">
...
/** Context-window usage meter shared by the desktop header and the mobile
 *  secondary-controls popover. */
function ContextUsageMeter({ contextUsage }: { contextUsage: ContextUsageSnapshot }) {
...
function ModelPicker({
  ...
  return (
    <div className={`relative flex items-center gap-1 text-xs text-neutral-500 ${className ?? ""}`}>
      ...
      <button
        type="button"
        data-testid="chat-model-picker-toggle"
        onClick={onToggle}
        ...
        className="max-w-40 sm:max-w-64 truncate rounded border border-neutral-800 bg-neutral-900 px-2 py-0.5 text-left text-xs text-neutral-200 outline-none hover:border-neutral-700 focus:border-blue-600 disabled:cursor-not-allowed disabled:opacity-60"
      >
        {selectedModelLabel}
      </button>
      {isOpen && (
          ...
          <div className="max-h-80 overflow-y-auto p-1">
            {!selectedModelKnown && selectedModelValue && (
              <button
                ...
                onClick={() => onSelect(selectedModelValue)}
              >
            ...
            {options.length === 0 ? (
              ...
              Array.from(new Set(options.map((model) => model.provider))).map(
                (provider) => (
...
export function ChatPanel({
  ...
  return (
    ...
    <section ref={chatPanelRootRef} className="h-full flex flex-col bg-neutral-950 text-neutral-100">
      {/* ---- Top bar ---- */}
      <header className="min-h-9 shrink-0 border-b border-neutral-800 px-2 py-1 flex flex-col gap-1 sm:min-h-10 sm:px-4 sm:py-2">
        {/* Primary row wraps rather than clipping so every control stays reachable on narrow screens. */}
        <div className="flex w-full flex-wrap items-center gap-x-2 gap-y-1">
        <div className="flex min-w-0 flex-1 items-center gap-1.5 sm:gap-2">
          <span className="min-w-0 max-w-[40vw] truncate text-xs font-mono text-neutral-400 sm:max-w-none">
            {activeSession?.cwd || "…"}
          </span>
          {currentGoal && (
            <span
              className={`text-[10px] px-1.5 py-0.5 rounded border truncate max-w-[140px] sm:max-w-[200px] ${
          ...
          {isMobile && <BashModeStatus mode={bashMode} compact />}
        </div>
        ...
        <div className="ml-auto flex shrink-0 items-center gap-1.5 sm:gap-2">
          ...
          {!isMobile && <TranscriptInspectorButton />}
          <button
            type="button"
          ...
          {!isMobile && contextUsage && <ContextUsageMeter contextUsage={contextUsage} />}
          <div className="flex items-center gap-1.5 text-xs text-neutral-500">
            <div className={`w-2 h-2 rounded-full ${statusDot}`} />
            {!isMobile && <span className="font-mono">{isCompacting ? "compacting" : wsStatus}</span>}
          </div>
          {isMobile && (
            <button
              type="button"
              data-testid="chat-mobile-controls-toggle"
              ...
              className="rounded border border-neutral-800 bg-neutral-900 px-2 py-0.5 text-xs text-neutral-300 hover:border-neutral-600 hover:text-neutral-100"
            >
              ⋯
            </button>
          )}
        ...
        {!isMobile && <BashModeStatus mode={bashMode} />}
        {!isMobile && visibleTodos.length > 0 && (
      ...
      >
        {transcriptState.mode === "window-v1"
          && (effectiveTranscriptOpenIntent.kind === "around" || (effectiveTranscriptOpenIntent.kind === "latest" && matchIntentRef.current)) && (
          <div className="sticky top-0 z-20 flex justify-center">
            {effectiveTranscriptOpenIntent.kind === "around" && (
              <button
                ...
                className="rounded border border-blue-800/70 bg-blue-950/40 px-2 py-1 text-[11px] font-semibold text-blue-200 hover:bg-blue-900/50"
              >
                Jump to latest
              </button>
            )}
            {effectiveTranscriptOpenIntent.kind === "latest" && matchIntentRef.current && (
              <button
                ...
                className="rounded border border-amber-800/70 bg-amber-950/30 px-2 py-1 text-[11px] font-semibold text-amber-200 hover:bg-amber-900/40"
              >
                Back to match
              </button>
            )}
        ...
        {transcriptState.mode === "window-v1" && transcriptState.hasOlder && (
          <div data-testid="transcript-gap-before" className="relative z-10 flex items-center justify-center gap-2 rounded border border-dashed border-neutral-700 bg-neutral-900/40 px-3 py-2 text-xs text-neutral-400">
        ...
        {transcriptState.mode === "window-v1" && transcriptState.hasNewer && (
          <div data-testid="transcript-gap-after" className="relative z-10 flex flex-wrap items-center justify-center gap-2 rounded border border-dashed border-blue-900/70 bg-blue-950/20 px-3 py-2 text-xs text-blue-200">
      ...
      {/* Auxiliary interaction surfaces (queued messages, approvals, interviews,
          sudo and guard prompts) share one bounded, scrollable dock. Without
          this, a tall queued card or questionnaire could squeeze the
          transcript to zero and push the composer under the mobile tab bar. */}
      <div
        data-testid="chat-interaction-dock"
        className="flex min-h-0 shrink flex-col overflow-y-auto overscroll-contain max-h-[62dvh] sm:max-h-[55dvh]"
      >
      ...
      <div className="border-t border-neutral-800 px-3 py-2 shrink-0">
        ...
        {pendingAttachments.length > 0 && (
          <div className="mb-2 flex max-h-[4.5rem] gap-2 overflow-x-auto overscroll-contain pb-1">
            {pendingAttachments.map((attachment) => (
              <div key={attachment.id} className="group relative shrink-0 overflow-hidden rounded border border-neutral-700 bg-neutral-900">
        ...
        <div className="relative flex items-end gap-2">
          ...
          <textarea
            ...
            className="min-w-0 flex-1 bg-neutral-900 text-sm text-neutral-100 rounded-lg px-3 py-2 resize-none border border-neutral-700 focus:outline-none focus:border-blue-500 placeholder-neutral-600"
            rows={1}
            placeholder={
              isAgentRunning
