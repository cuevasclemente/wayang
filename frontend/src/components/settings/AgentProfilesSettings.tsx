...
export function AgentProfilesSettings({ profiles, workspaceSettings, models, onProfilesChanged }: AgentProfilesSettingsProps) {
  const [selectedId, setSelectedId] = useState<string | null>(profiles[0]?.id ?? null);
  const [draft, setDraft] = useState<Draft | null>(null);
  const [references, setReferences] = useState<AgentProfileReferenceSummary | null>(null);
  const [workspaceDefaultId, setWorkspaceDefaultId] = useState(workspaceSettings.default_agent_profile_id);
  ...
  const selectedIsWorkspaceDefault = draft?.id === workspaceSettings.default_agent_profile_id;
  ...
  useEffect(() => {
    setWorkspaceDefaultId(workspaceSettings.default_agent_profile_id);
  }, [workspaceSettings.default_agent_profile_id]);
  ...
  useEffect(() => {
    ...
    setSaved(false);
    setReferences(null);
    void Promise.all([fetchAgentProfile(selectedId), fetchAgentProfileReferences(selectedId)]).then(([profile, profileReferences]) => {
      if (!cancelled) {
  ...
  }, [selectedId, workspaceSettings.default_agent_profile_id]);

  const patchDraft = (patch: Partial<Draft>) => setDraft((current) => current ? { ...current, ...patch } : current);

  const saveWorkspaceDefault = async () => {
    if (workspaceDefaultId === workspaceSettings.default_agent_profile_id) return;
    setWorkspaceSaving(true);
    setWorkspaceSaved(false);
    setError("");
    try {
      await updateWorkspaceSettings(workspaceDefaultId);
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-4 md:flex-row">
      <aside className="w-full shrink-0 border-b border-neutral-800 pb-4 md:w-56 md:border-b-0 md:border-r md:pb-0 md:pr-4">
        ...
        <div className="flex gap-2 overflow-x-auto md:block md:space-y-1 md:overflow-visible">
          {profiles.map((profile) => (
            ...
            >
              <div className="flex items-center gap-2"><Bot size={14} className="shrink-0" /><span className="min-w-0 flex-1 truncate text-sm font-medium">{profile.name}</span>{profile.id === workspaceSettings.default_agent_profile_id && <span className="rounded bg-blue-950 px-1.5 py-0.5 text-[9px] uppercase text-blue-300">default</span>}{!profile.enabled && <span className="text-[9px] uppercase text-neutral-600">off</span>}</div>
      ...
      <div className="min-w-0 flex-1 overflow-y-auto pr-0 md:pr-2">
        <div className="mb-6">
          <SettingsSection title="Workspace default" description="Fallback Agent Profile for future or newly registered Projects that do not choose an explicit profile. Changing it does not switch current sessions, alter existing Project defaults, or rewrite session attribution.">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
              <div className="min-w-0 flex-1">
                <Field label="Default agent profile">
                  <select
                    aria-label="Workspace default agent profile"
                    value={workspaceDefaultId}
                    onChange={(event) => { setWorkspaceDefaultId(event.target.value); setWorkspaceSaved(false); }}
              ...
              <button
                ...
                disabled={workspaceSaving || workspaceDefaultId === workspaceSettings.default_agent_profile_id}
                className={primaryButtonClass}
              >
                {workspaceSaving ? <Loader2 size={14} className="animate-spin" /> : <Save size={14} />}
                Set workspace default
              </button>
            </div>
            {workspaceSaved && <span className="inline-flex items-center gap-1 text-xs text-emerald-400"><Check size={13} /> Workspace default updated without changing existing attribution.</span>}
          </SettingsSection>
        </div>
        {loading ? (
          ...
          <form onSubmit={save} className="space-y-6 pb-6">
            <SettingsSection title={draft.id ? "Agent profile" : "Create agent profile"} description="The stable profile ID—not its name, definition, provider, or model—is the identity evaluated by Project allowlists and privacy-derived authority.">
              ...
              {draft.id && (
                <>
                  <label className="flex items-center gap-2 text-xs text-neutral-300"><input type="checkbox" checked={draft.enabled} disabled={selectedIsWorkspaceDefault && draft.enabled} onChange={(event) => patchDraft({ enabled: event.target.checked })} className="accent-blue-500 disabled:opacity-50" /> Enabled</label>
                  <div className="rounded border border-amber-900/60 bg-amber-950/20 px-3 py-2 text-xs leading-relaxed text-amber-100">
                    Runtime authority is derived from each Project's privacy mode and allowlist for this stable profile ID. Renaming it or changing instructions/defaults does not change authority. Disabling blocks it everywhere; re-enabling restores authority wherever the profile remains allowed. {selectedIsWorkspaceDefault ? "Set another workspace default above before disabling this profile." : "Choose another Project default first where required."}
                  </div>
                </>
              )}
            </SettingsSection>
            ...
            {draft.id && references && (
              <SettingsSection title="Profile references" description="Redacted counts separate active/configuration use from preserved stable-ID attribution. Session attribution may remain after disable and is never rewritten by changing the workspace default.">
                <dl className="grid gap-2 text-xs sm:grid-cols-2">
                  <ReferenceCount label="Workspace default" value={references.workspace_default ? "yes" : "no"} />
                  <ReferenceCount label="Project defaults" value={references.project_defaults} />
                  <ReferenceCount label="Project allowlists" value={references.project_allowlists} />
                  <ReferenceCount label="Running sessions" value={references.running_sessions} />
                  <ReferenceCount label="Persisted session attribution" value={references.session_attributions} />
                  <ReferenceCount label="Pending switches" value={references.pending_switches} />
                  <ReferenceCount label="Scheduled jobs" value={references.scheduled_jobs} />
                  <ReferenceCount label="Protected automation jobs" value={references.protected_automation_jobs} />
                  <ReferenceCount label="Protected automation history" value={references.protected_automation_runs} />
                  <ReferenceCount label="Messaging endpoints" value={references.messaging_endpoints} />
                </dl>
              </SettingsSection>
            )}
...
function ReferenceCount({ label, value }: { label: string; value: number | string }) {
  return (
    <div className="flex items-center justify-between gap-3 rounded border border-neutral-800 bg-neutral-950/60 px-3 py-2">
