...
function activationRequestErrorMessage(error: unknown, timedOut: boolean): string {
  ...
  if ((error.status === 409 || error.status === 413) && code === "runtime_limit") {
    return "Capability review found more affected runtimes than the bounded review can display. Reduce the affected runtime count and retry; this is a runtime-count limit, not a capability state conflict.";
  }
  if (error.status === 409) {
    return code === "realm_busy"
      ...
      : "Capability authority state changed while creating the review. Refresh Settings and retry the exact association; affected runtime activity alone does not require idle sessions or a new review.";
  }
...
function affectedRuntimeStatusLabel(
  status: WorkspaceCapabilityChallenge["affectedRuntimes"][number]["status"],
): string {
  return status === "mutation_locked" ? "mutation locked" : status;
}
...
export function WorkspaceCapabilitiesSettings({
  ...
  const commitChallenge = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setNotice("Capability associated with the displayed Project-Agent pair. New runtimes receive the grant immediately. Current and already-queued work on an older runtime finishes with its old authority; later work uses a fresh runtime with the grant.");
      await refresh();
      onChanged?.();
    } catch (caught) {
    ...
    <div ref={contentRef} className="min-h-0 flex-1 overflow-y-auto pb-6">
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        <form onSubmit={requestActivation} aria-busy={requesting} className="space-y-4 rounded-lg border border-neutral-800 bg-neutral-900/35 p-4">
          <div><h3 className="text-sm font-semibold text-neutral-100">Associate capability</h3><p className="mt-1 text-xs leading-relaxed text-neutral-500">The backend will render a fresh review of this exact Project, Agent Profile, and capability before accepting a one-use PIN submission. Affected runtimes may be busy; their status is review information and does not require stopping current work.</p></div>
