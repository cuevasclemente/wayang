...
> **Wayang controls a powerful local agent. It can expose project files, command execution, browser profiles, provider access, and transcripts. Project allowlist/protected-mode guards and per-exec filesystem sandboxing are targeted controls, not a general same-user or network sandbox. Sandboxed bash has unrestricted proxy-mediated outbound TCP access. A PIN-approved exact Project-Agent pair can receive Standard resources plus the generic masked-host workspace, including scheduled ordinary-host filesystem/Unix-IPC access while registered Protected roots stay masked; direct host execution and authenticated-browser capabilities are broader cooperative powers of the Wayang OS user, not containment. Historical compatibility is additionally inert unless this deployment has an explicit owner-provisioned local activation witness. Wayang is not safe for mutually untrusted users or public Internet exposure.**
