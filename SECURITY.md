...
- enforces every registered project agent allowlist plus protected-mode policy for participating path tools, and routes standard and restricted-profile bash through an independent OS filesystem sandbox when supported unless an exact reviewed project+profile assignment grants host execution; control sockets are blocked except that the exact seeded legacy Wren profile retains Unix IPC for eligible interactive Standard-project runtimes, while remaining filesystem-sandboxed; sandboxed bash can write its current project and shared host temporary storage while explicit protected roots remain denied, but neither sandbox mode nor capability assignment isolates temporary files, arbitrary same-UID processes, visible same-user socket services, or trusted in-process extensions from the host user;
...
PIN-backed capability authority is identity-neutral. Its sole live durable authority is an approved association between one immutable Project ID and one stable Agent Profile ID. Project privacy mode, profile enabled state, and the project's exact-profile allow decision are rechecked. Provider/model are fluid runtime choices and never confer, narrow, or revoke capability authority. Names, prompts, and legacy environment keys do not confer capability authority; cloning a profile creates a different stable ID and does not transfer associations. Separately, for compatibility with pre-policy Wayang, only the exact migration-seeded Wren stable ID plus its non-user-settable historical kind retains Unix-socket access while filesystem-sandboxed in interactive Standard projects. Renames and clones do not acquire that narrow IPC compatibility, and it is never host execution.
...
- `WAYANG_DATA_DIR/workspace-capability-approval/pin-attempt-state.json`, the private capability-approval attempt/cooldown record (it contains no PIN);
