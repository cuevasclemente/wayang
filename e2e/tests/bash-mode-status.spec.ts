...
type RuntimeMode = "host" | "sandboxed" | "masked-host-workspace" | "unavailable" | "wren_host";
...
test("renders generic host authority and rejects legacy or stale runtime claims", async ({ page }) => {
  ...
  await emitRuntimeState(page, "masked-host-workspace");
  await expect(status).toHaveAttribute("data-bash-mode", "masked-host-workspace");
  await expect(status).toContainText("Masked host workspace");
