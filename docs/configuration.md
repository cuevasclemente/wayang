...
Required arguments are `--source`, `--output`, `--catalog-proof`, and `--catalog-proof-sha256`. The proof path selects the reviewed `source-provenance.json`, with sibling `derivation.json` and `data/`. The hash must be the full lowercase SHA-256 independently recorded during proof review—not a newly computed value used to bless whichever bytes happen to occupy that path. Proof bytes are hash-checked before JSON parsing or packing; source revision, deriver/report identities, verification evidence, and the exact source/build/catalog file inventories must agree.
...
Core and AI keep package version `0.85.0`; the SDK becomes `0.85.0-wayang.<source-prefix>`. SDK/core filenames retain the eight-character source prefix, while AI's filename uses the first eight characters of its final tarball SHA-256. All three packages carry the full source revision and catalog-manifest hash; SDK/core carry their required peer source revisions, and AI carries the reviewed derivation, provenance, archive-input, and deriver hashes. Dependency/peer metadata and the complete SDK shrinkwrap graph are preserved; only the SDK package version and shrinkwrap top/root versions change. If the parent's exact-triplet install rehearsal finds nested substitutes or incompatible sharing, stop for a reviewed adaptation rather than deleting the shrinkwrap or assuming version equality proves sharing.
