...
Wayang uses pi 0.85.0's standard authentication and model runtime. The coding-agent package is pinned to the repository-vendored `0.85.0-wayang.fa40ae91` artifact (SHA-256 `e4237a2fb91c92c5ae9da3b37bf4ddd35572ca4e36d1dcc61f1aa64e66b4fb43`) so session-name writers and PIN-gated transcript event mutations share the reviewed physical-file transaction, lock, atomic multi-entry CAS, stale-runtime mutation epoch, optional fixed compaction threshold, optional complete-turn retention, and incremental resume-session loading. This revision also fences cancelled prompt preflight, exposes explicit pending-invocation drainage, and supports session-only model selection without changing deployment defaults, and retries transient upstream/stream transport errors (for example an HTTP/2 body-read failure a gateway reports after the response status is already committed) while keeping deterministic quota/billing limits non-retryable. The paired repository-vendored agent-core artifact is `earendil-works-pi-agent-core-0.85.0-wayang.fa40ae91.tgz` (SHA-256 `9484dba8364ffb89bc881b97011cbfd3194dfc754a5115e1e0db53d3da946d10`). The model runtime is pinned to the repository-vendored Pi AI `0.85.0-wayang.22ca19ed` artifact (SHA-256 `22ca19ede1015ad1247309efbe79f6558dfa278f44056469bd0a8c8c1082490f`), preserving the reviewed GPT-6 Astra catalog additions; the Codex entry remains an unverified OAuth placeholder, not an account-access claim. Core and AI retain upstream package version `0.85.0`. All three artifacts bind source revision `fa40ae91ad733a8a0bc369cfbdcaca381fa47a46` and catalog manifest SHA-256 `e6dd5f432d502e84981ac15c14d9eb0f78bab7baf1aac8dbb805d48b8b3c3655`. SDK/core source-prefix filenames and AI's content-prefix filename are locators, not integrity proof: committed locks must match the exact tarball SHA512 SRI. The triplet must resolve together for exact custom-message discard receipts and model-runtime identity, including the SDK's declared Pi server dependency and its paired core resolution. Modular imports remain at `dist/index.js`; the declared CLI and exported RPC use `dist/bundle/cli.js` and `dist/bundle/rpc-entry.js` and require separate installed-entrypoint verification. Source/artifact updates do not restart a live backend or change host-local settings, authentication, or identity.

### Maintainer-only prebuilt Pi 0.85 artifact production

`scripts/vendor-pi-artifacts.py` produces a coordinated **AI/core/SDK triplet** from an already checked, offline-built, clean Pi `0.85.0` source tree. Both the initial and prepublication cleanliness checks reject tracked changes and nonignored untracked files; Git-ignored build outputs and dependencies remain allowed. This producer workflow does not change the installed pins described above, install dependencies, rebuild, regenerate catalogs, publish packages, or activate a host. Never run Pi's live-release/model-generation scripts as a substitute.
...
The producer invokes only `npm pack --ignore-scripts` after validation. It snapshots publishable source files, requires modular runtime/type exports, the SDK's declared bundled CLI and exported RPC, bundled shared/lazy implementations, and copied runtime assets, then compares the packed bytes and rechecks inputs before writing destinations. Only the SDK's intentional `./experimental/plugin` source-only export condition is exempt from shipped-target checks; its runtime/type targets are required. Reads are bounded: 32 MiB per ordinary file, 8 MiB per JSON/command result, 100 MiB per compressed or expanded archive/package snapshot, and 10,000 entries per snapshot/archive. These limits do not replace the separate installed modular/bundled-entrypoint and native-platform gates.
