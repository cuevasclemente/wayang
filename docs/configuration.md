...
Listed workspace capabilities are assigned through a PIN-approved association between one immutable Project ID, one stable Agent Profile ID, and one compiled capability ID—not through a profile name or provider/model. Project privacy mode, profile enabled state, the project's exact-profile allow decision, and any deployment-local historical-profile activation must match. Renaming or editing a stable profile preserves its associations; copying creates a new ID and transfers none. Broad masked-host workspace authority requires both `wayang.standard-resources.v1` and `wayang.masked-host-workspace.v1` for the exact Standard pair and is distinct from direct host execution. Existing historical compatibility is a temporary pre-cutover fallback available only on an explicitly activated deployment.
...
### Historical-agent deployment activation

Normal Wayang setup creates no historical agent and no activation witness. Fresh and schema-0 stores use a generated restricted Default profile. Existing current-schema stores may retain a historical profile row, but every project action and capability resolution for that exact row is inactive unless two owner-private files under `${XDG_CONFIG_HOME:-~/.config}/wayang/` match this deployment:

- `deployment-id`
- `historical-agent-activation.json`

Both are mode `0600` regular single-link files outside `WAYANG_DATA_DIR`; ordinary store/data-directory copies therefore do not carry activation. The record contains only a random deployment ID, the historical stable profile ID, a revision, and a timestamp—no prompt, memory, PIN, provider credential, or autobiography.

Provisioning is **not** normal installation. Run it only on the explicitly selected active historical-agent home, before deploying code that enforces the witness:
...
`make doctor` reports only whether activation metadata is active, normally absent, or unsafe. Activation is captured for the process lifetime; restart Wayang after provisioning or changing the record.
...
| `~/.wayang/store.json` | Persistent Wayang session metadata, capability associations, monotonic historical-pair cutover markers, jobs and schedules, apps, and UI state. Current schema 8 adds empty cutover state on migration and creates no authority. |
...
| `${XDG_CONFIG_HOME:-~/.config}/wayang/deployment-id` | Owner-private random local installation identifier outside portable Wayang store backups. |
| `${XDG_CONFIG_HOME:-~/.config}/wayang/historical-agent-activation.json` | Optional owner-provisioned deployment-bound historical-profile activation metadata; absent on normal/fresh/secondary deployments. |
