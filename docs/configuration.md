...
Wayang uses pi 0.85.1's standard authentication and model runtime. The coding-agent package is pinned to the repository-vendored `0.85.1-wayang.4c47f7f2` artifact (SHA-256 `90695a28f0eb8dfeab82b6f249a57a151f833ba22c0bd0eb1d203357ae6eec96`) so session-name writers and PIN-gated transcript event mutations share the reviewed physical-file transaction, lock, atomic multi-entry CAS, stale-runtime mutation epoch, optional fixed compaction threshold, optional complete-turn retention, and incremental resume-session loading. This revision also fences cancelled prompt preflight, exposes explicit pending-invocation drainage, and supports session-only model selection without changing deployment defaults, and retries transient upstream/stream transport errors (for example an HTTP/2 body-read failure a gateway reports after the response status is already committed) while keeping deterministic quota/billing limits non-retryable. The paired repository-vendored agent-core artifact is `earendil-works-pi-agent-core-0.85.1-wayang.4c47f7f2.tgz` (SHA-256 `1345f9b28453f7a9695580162a3f11f2606a67300b8a4857f4167806aa3b353f`). The model runtime is pinned to the repository-vendored Pi AI `0.85.1-wayang.ec9c86b2` artifact (SHA-256 `ec9c86b262bd4fb40423d5768be6c656ca2a61ad8923fdeffc6f8d87dc2d4ec5`), preserving the reviewed GPT-6 Astra catalog additions; the Codex entry remains an unverified OAuth placeholder, not an account-access claim. Core and AI retain upstream package version `0.85.1`. All three artifacts bind source revision `4c47f7f24b59981b0b41c6b43bbff68dca74e08c` and catalog manifest SHA-256 `e6dd5f432d502e84981ac15c14d9eb0f78bab7baf1aac8dbb805d48b8b3c3655`. SDK/core source-prefix filenames and AI's content-prefix filename are locators, not integrity proof: committed locks must match the exact tarball SHA512 SRI. The triplet must resolve together for exact custom-message discard receipts and model-runtime identity, including the SDK's declared Pi server dependency and its paired core resolution. Modular imports remain at `dist/index.js`; the declared CLI and exported RPC use `dist/bundle/cli.js` and `dist/bundle/rpc-entry.js` and require separate installed-entrypoint verification. Upstream 0.85.1 ships no bundled `client`/`coordinator` entries: those subpaths are monorepo-only source conditions, and the required bundle entries are `dist/bundle/cli.js`, `dist/bundle/rpc-entry.js` and `dist/bundle/index.js`. Source/artifact updates do not restart a live backend or change host-local settings, authentication, or identity.

### Maintainer-only prebuilt Pi 0.85 artifact production

`scripts/vendor-pi-artifacts.py` produces a coordinated **AI/core/SDK triplet** from an already checked, offline-built, clean Pi `0.85.1` source tree. Both the initial and prepublication cleanliness checks reject tracked changes and nonignored untracked files; Git-ignored build outputs and dependencies remain allowed. This producer workflow does not change the installed pins described above, install dependencies, rebuild, regenerate catalogs, publish packages, or activate a host. Never run Pi's live-release/model-generation scripts as a substitute.

Required arguments are `--source` and `--output`. The optional `--catalog-proof`/`--catalog-proof-sha256` pair selects a reviewed `source-provenance.json`, with sibling `derivation.json` and `data/`, and must be supplied together or not at all. When supplied, the hash must be the full lowercase SHA-256 independently recorded during proof review—not a newly computed value used to bless whichever bytes happen to occupy that path—and proof bytes are hash-checked before JSON parsing or packing. Wayang no longer requires a proof: with the pair omitted, the producer binds the packed catalog bytes directly through the source/build catalog inventory and manifest hash, so a clean, committed, offline-built source at a pinned revision is deployable once its tests pass.

The producer invokes only `npm pack --ignore-scripts` after validation. It snapshots publishable source files, requires modular runtime/type exports, the SDK's declared bundled CLI and exported RPC, bundled shared/lazy implementations, and copied runtime assets, then compares the packed bytes and rechecks inputs before writing destinations. Only the SDK's two reviewed monorepo-only source conditions (`./client` and `./experimental/plugin`) are exempt from shipped-target checks, and they declare no runtime/type targets; the bundled entries required of the pack are `dist/bundle/cli.js`, `dist/bundle/rpc-entry.js` and `dist/bundle/index.js`. Reads are bounded: 32 MiB per ordinary file, 8 MiB per JSON/command result, 100 MiB per compressed or expanded archive/package snapshot, and 10,000 entries per snapshot/archive. These limits do not replace the separate installed modular/bundled-entrypoint and native-platform gates.

The npm record envelope may be a singleton array or npm 12's singleton package-name-keyed object; the object key must match both the expected package and record identity. Raw archive hashes and all supplied member bytes are checked before transformation. Declared regular CLI files are sealed as executable without changing their content or source permissions. npm 12 excludes `npm-shrinkwrap.json`; only that absent SDK member may be restored from the exact bounded, prevalidated source snapshot at mode `0644`. Existing invalid locks are never replaced, and every other required member remains mandatory. This is source-bound metadata restoration, not lock regeneration or proof that an installer honors the graph.

Core and AI keep package version `0.85.1`; the SDK becomes `0.85.1-wayang.<source-prefix>`. SDK/core filenames retain the eight-character source prefix, while AI's filename uses the first eight characters of its final tarball SHA-256. All three packages carry the full source revision and catalog-manifest hash; SDK/core carry their required peer source revisions. Dependency/peer metadata and the complete SDK shrinkwrap graph are preserved; only the SDK package version and shrinkwrap top/root versions change. If the parent's exact-triplet install rehearsal finds nested substitutes or incompatible sharing, stop for a reviewed adaptation rather than deleting the shrinkwrap or assuming version equality proves sharing.

Successful stdout is a JSON receipt with `sourceRevision` and three `artifacts`, each containing `file` (canonical basename), `sha256`, `integrity` (npm SHA512 SRI), and `sizeBytes`. Basenames are locators, not integrity proof. The output must be outside source/catalog inputs. Every destination is checked before any artifact is written; identical existing bytes are reusable, but different bytes, symlinks, and non-regular destinations are refused. Writes use exclusive creation. This assumes stable caller-controlled parent directories, not hostile same-UID race isolation or a filesystem-wide atomic triplet transaction.

Packing failures retain their newly created staging tree and print its path to stderr; do not overwrite or silently clean failed outputs. Only that invocation's successful staging tree is removed. Preserve old artifacts for rollback, compare repeat-produced bytes, and complete the parent's isolated install/consumer/runtime validation before changing pins or activating either host.
...
OpenRouter models are intentionally unavailable in Wayang's picker and model-switch API. The automatic and explicit session-title generator does not use these lanes: it pins its own zero-data-retention-routed descriptor on the built-in `openrouter` provider, so it never depends on user-editable `models.json`. The OpenRouter credential and pi provider are not deleted, so this is reversible and does not affect other pi clients. Wayang may still consult OpenRouter's public, unauthenticated model catalog to derive current direct Anthropic identifiers; no prompt, transcript, credential, or private content is sent in that metadata-only request.
...
### Automatic DeepSeek V4.1 session titles
...
| `WAYANG_AUTO_TITLE_PROVIDER` | `openrouter` | Host-local selection between the two reviewed, immutable title descriptors: `openrouter` (alias `deepseek`, the default OpenRouter DeepSeek V4.1 ZDR descriptor) or `codex` (alias `terra`, the opt-in OpenAI Codex/Terra descriptor). Unset or empty preserves the OpenRouter DeepSeek default. Any other value fails closed with `title_model_unavailable`. |

The operator selects one of exactly two reviewed providers host-locally through `WAYANG_AUTO_TITLE_PROVIDER`; the default remains OpenRouter DeepSeek (`openrouter/deepseek/deepseek-v4.1-flash`), routed with pinned zero-data-retention preferences (`zdr: true`, `allow_fallbacks: true`, `require_parameters: true`). There is no provider fallback: an unreviewed selector value fails closed and never substitutes another provider. Both descriptors are immutable and pinned to their provider, model id, API, base URL, and credential requirements, so the outgoing request is always built from the reviewed object. models.json keys/commands, extension providers, and runtime overrides are never used for this path. Credentials must come from the selected provider's reviewed built-in authentication: the OpenRouter credential in pi `auth.json` or the operator environment for `openrouter`, or the built-in `openai-codex` OAuth credential for `codex` (a non-OAuth or unreviewed credential source is rejected). Disclosure authorization semantics are unchanged; the selected provider is the disclosure target. Title requests disable model reasoning (the Codex descriptor pins `minimal`) so the bounded output budget is spent on the title itself. The immediate request contains only deterministically bounded raw browser text from the first physically persisted user message, captured before Wayang goal/attachment decoration; it does not wait for assistant settlement. If that attempt fails, a later eligible browser interaction may retry using bounded prose from the first one to three completed exchanges, including assistant text blocks. Wayang does not inject tools, tool results, reasoning, images, attachment metadata/bytes, host paths, project files, profile/system instructions, later-turn prose, or source-session IDs. Conversation prose can itself contain sensitive facts, paths, or credentials written by the human or repeated by the assistant. Enabling either scope authorizes disclosure of that prose to the provider; the Protected flag is a separate explicit decision.
...
The session-row **Generate title** action uses the same configured DeepSeek V4.1 disclosure scope and is unavailable when the corresponding flag is off. After confirmation, it may replace any existing title using bounded prose from the first one to three completed active-branch exchanges. A request made while the session is busy is queued in backend memory, survives browser navigation/disconnect, and runs once the session is idle; it is intentionally lost on a Wayang restart. A newer manual or physical Pi rename wins instead of being overwritten. Archive, delete, or manual rename cancels pending work.
