...
Project-Agent runtime authority is derived from Project privacy and RBAC; there is no capability approval UI. Wayang still reuses the command guard's existing identity PIN for separate operation-specific confirmations. The deployed service initializes missing non-secret attempt/cooldown state owner-privately when safe. `make setup-owner-pin-confirmations` is an optional human-run metadata preflight. The agent must not create a PIN, inspect either file, or substitute ad-hoc shell redirection.
