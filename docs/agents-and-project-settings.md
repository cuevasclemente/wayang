...
Every interactive Wayang runtime receives a model-independent host-layer communication appendix: briefly orient through relevant authorized memory/current project records before the first visible response when context is missing or stale, then acknowledge substantive work before extended research or implementation. Orientation is quiet, uses already-present context without redundant retrieval, never delays urgent safety guidance, and grants no additional memory access or provider-egress permission. Unavailable or prolonged retrieval calls for a specific statement of the gap rather than speculation. Longer work receives concise checkpoints around material findings, decisions, blockers, and useful steering points. It is appended without replacing profile, global, project, or `APPEND_SYSTEM.md` instructions. Scheduled runs do not receive it, and quick mechanical tasks should not add ceremonial updates. This is model guidance rather than a deterministic heartbeat; Wayang does not fabricate assistant messages or inject timer-based steering.
...
The model picker and the `/model` command work while the agent is streaming or has queued work. A busy runtime switches models live: the current turn finishes on the old model (pi captures the model once per run), and the next turn — a queued follow-up or the next prompt — uses the new model. The switch is validated first (model exists, auth configured, memory-first context-window requirements), appends a durable `model_change` transcript entry, and never rebuilds the runtime, so tools, approvals, browser leases, and queues stay intact. Wayang uses Pi's session-only model switch (`persist: false`): authentication checks, transcript changes, thinking-level clamping, and model-selection hooks remain active, but neither provider/model nor thinking defaults are written. Concurrent deliberate default edits remain intact; Standard resource discovery and later file-backed settings reload are unchanged.
