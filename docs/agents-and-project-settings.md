...
The Protected-automation implementation is complete through Milestone 5 with `activationAvailable: true`. A service restart is needed only to deploy new code, not to initialize attempt/cooldown state or record an approval. The flow reuses the command guard's existing identity PIN, and PIN entry is the normal human action for approving an exact reviewed Project–Agent pair. Startup automatically creates missing non-secret attempt/cooldown state under `WAYANG_DATA_DIR` with owner-only permissions and preserves it across reboots. Missing or unsafe PIN metadata and unsafe or malformed existing state fail approval closed without replacement. `make setup-capability-approval` is optional manual preflight/migration, not required setup. A runtime constructed after approval receives the production `protected_automation` tool immediately. A runtime that already existed when approval committed never gains the tool in place: its current turn and work already queued before commit finish with the old authority, then the stale handle retires and later work lazily constructs a fresh eligible runtime. No environment flag or provider/model choice bypasses approval. This documentation update does not deploy code or approve an association.

The source-session-bound tool operates only on its implicit immutable Project ID–Agent Profile ID and current association revision. It supports bounded reads; immutable source capture and compare-and-set update/tombstone/rebind; enable/pause; run-now/cancel; run history; exact browser-profile preparation; and a bounded expiring purge intent for an already tombstoned, fully stopped job. The separate owner UI is status/attention plus denial-only emergency pause/cancel, preparation attachment, and PIN-approved purge of an already tombstoned, fully stopped job. Preparation viewing normalizes only snapshotted blank startup targets, revokes later popup/target drift, navigates through explicit configured HTTPS-origin buttons, and offers a human-only bounded paste capture that is never stored in React state, logs, project files, or tool parameters. **Save & close preparation** cleanly stops Chromium, writes a private non-secret last-saved marker only while exact authority remains current, and projects saved/not-saved plus the timestamp without claiming the site login is still valid. An agent purge intent never accepts PIN material or deletes anything: the owner must still review the exact tombstoned job and enter the existing identity PIN in Wayang, and the existing purge preserves Project outputs. It does not offer ordinary owner create/edit/enable/run controls.
...
Choose the Project privacy mode and allowed-agent list first, then review and associate only the intended capability and Project-Agent pair. Provider/model defaults remain runtime preferences, not authority. Capability associations persist in the service store; the normal human approval action is entering the existing command-guard identity PIN for the exact reviewed association. The review may list affected runtimes as idle, streaming, queued, starting, or mutation-locked. Those statuses are informational and may change without invalidating the review: busy runtimes are expected and do not need to be stopped before approval. Reviews display at most 64 affected runtimes; exceeding that bound is reported as a distinct runtime-count limit rather than a stale capability-state conflict.

On approval, the durable association is active immediately and every newly constructed runtime receives it. A runtime published before commit keeps its captured old tools, resources, execution mode, and leases for its current turn and work already queued before commit. It accepts no later top-level work, retires after accepted work settles, and is replaced lazily by a coherent fresh runtime. Runtime construction that overlaps activation cannot publish a mixed old/new authority set and must retry. This is a participating-runtime lifecycle guarantee, not isolation from cooperative same-user processes: host execution can inspect same-user state, and network-enabled bash may forge loopback UI requests in passwordless-loopback deployments.

Explicit revocation, exact-profile allowlist exclusion, incompatible privacy, or subject deletion tombstones the association. Profile definition edits, project instructions/defaults, and provider/model changes preserve it; stale runtime objects are still destroyed and rebuilt. Revocation remains denial-first and immediate: it synchronously denies future dispatch and terminates affected direct-host command and Protected-automation process groups with bounded TERM/KILL. Deferred activation does not delay or weaken revocation, which still cannot undo host commands, Project writes, browser actions, downloads, disclosures, or remote side effects already completed.
...
Projection publication is denial-first. Before any store mutation can become durable, every affected host-execution pair is replaced with an owner-private `available: false` root. Only after the store rename and directory fsync does Wayang publish a fresh positive projection; a post-commit projection failure leaves the denial in place. Startup likewise writes denial roots and rebuilds from the validated store before serving. Capability revocation or pair unavailability leaves the root unavailable with no questionnaire subsection. Consumers must require the documented schema versions, the expected current association revision, an active/available root, and an available questionnaire subsection; missing, malformed, unknown-version, unavailable, stale, or partial data is not evidence of a submission.
...
Profile, project-policy, default, or `AGENTS.md` edits still refuse to interrupt streaming/queued work. Affected idle runtimes stop and rebuild on the next prompt. PIN-approved capability activation is the deliberate exception: busy accepted work finishes on its captured old runtime authority, later work is fenced to a fresh runtime, and newly constructed runtimes for the pair receive the grant immediately.
...
Approved interactive browser access is backend-owned and requires the exact active `wayang.standard-browser.v1` or `wayang.protected-browser.v1` assignment compatible with the Project privacy mode. New runtimes receive explicit `browser_*` tools immediately; runtimes published before activation finish only current and already-queued work with their captured old tool set, then later work uses a fresh runtime. Scheduled/background sessions receive no interactive browser tools. Each operation reauthorizes the exact source Project, Agent Profile, association revision, runtime generation, and process generation. Once granted, browser authority is broad and may mutate authenticated sites; the capability check is not an operation-level read-only policy. Apps companion calls retain their separate source-session-attributed integration. App manifest commands remain same-user unsandboxed processes, so agent start/restart fails closed whenever any Protected project exists; manual authenticated launch remains available after human review.
