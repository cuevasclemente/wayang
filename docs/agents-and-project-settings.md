...
Fresh installations and schema-0 migrations seed one restricted **Default** profile with a generated stable ID, project-only resources, and no memory or privileged capability grants. Newly discovered projects use that profile. Existing historical Wren and Neutral rows may remain in already-current stores as ordinary retained data. Neither name grants authority. The historical row is inert unless an owner explicitly provisions a matching deployment-local activation witness outside `store.json`; copying or restoring the store alone cannot activate it. During staged migration, activated uncut historical pairs retain compatibility so authority is not accidentally narrowed. A monotonic pair cutover switches them to generic `wayang.standard-resources.v1` plus `wayang.masked-host-workspace.v1`; after cutover, revocation cannot restore historical fallback. Every registered Protected project and protected backing artifact remains masked.
...
- `wayang.standard-resources.v1` for a Standard Project-Agent pair. It permits Pi global instructions, skills, prompts, reviewed extensions, and ordinary Standard direct-tool policy for any provider/model used by that agent in the project.
- `wayang.masked-host-workspace.v1` for a Standard Project-Agent pair that also has Standard resources. It permits broad ordinary-host filesystem/Git access and visible Unix IPC through the sandbox, including eligible scheduled sessions, while Protected/control-plane roots remain masked. It is not direct host execution.
...
Wayang sessions replace Pi's bash backend with a fresh per-command OS sandbox unless the exact Standard Project-Agent pair has an active `wayang.host-execution.v1` association. Provider/model changes preserve that association but destroy stale runtime handles before rebuilding. Ordinary restricted profiles in Standard projects remain project-scoped. A restricted profile in its authorized Protected project receives asymmetric access: ordinary and Standard paths are readable, its own project is writable, and every other Protected/control-plane root remains masked. An exact Standard pair with both Standard resources and masked-host workspace uses a broader sandbox view with ordinary host reads/writes, global Pi resources, Git configuration, and Unix IPC, including eligible scheduled runs, while all registered Protected projects and protected backing artifacts remain masked. Activated historical uncut pairs temporarily retain that same behavior until generic cutover. Neither mode is direct host execution. The sandbox:

- masks unauthorized projects plus Pi sessions/transcripts, Wayang data, attachments from other sessions, browser profiles, and the command-guard identity PIN; masked-host workspace authority still masks every Protected project regardless of allowlist;
- enforces memory write mode for ordinary restricted profiles;
- permits ordinary Standard restricted writes inside the current project and shared host temporary storage; Protected runtimes persist writes only inside their current project; masked-host workspace permits ordinary host writes while protected host backing remains hidden and unmodifiable;
...
- blocks Unix-domain sockets for ordinary restricted profiles, permits them for an exact masked-host workspace pair, and blocks raw `sudo` for all profiles;
