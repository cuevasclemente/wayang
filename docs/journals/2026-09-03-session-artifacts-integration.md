...
The first GitHub CI run caught one clean-profile test regression before merge: the rebased browser-runtime expectation listed `memory_review_complete` unconditionally even though the existing test already adds that optional tool only when the memory-first review cohort is enabled. Runtime behavior was correct and `present_artifact` was present. The duplicate unconditional expectation was removed, the exact test passed with memory-first controls explicitly disabled, the focused 13-test artifact/upload/runtime suite passed again, and the full clean-default release gate passed again.
