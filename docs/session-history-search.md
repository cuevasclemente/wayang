...
If automatic backfill or refresh work harms service availability, `WAYANG_SEARCH_BACKGROUND_INDEXING=0` pauses boot backfill, periodic transcript refresh, and immediate changed-session indexing after restart. Existing indexed results remain readable and explicit manual reindex remains available. A lightweight policy-only heartbeat continues refreshing the durable authorization projection without scanning transcript files. Actual policy changes still trigger denied-entry purge, and query-time authorization filters stale indexed entries fail-closed even if purge is delayed. Search health exposes `watcher.background_indexing_enabled` and `watcher.policy_projection_available`; remove the override and restart to resume automatic indexing.
